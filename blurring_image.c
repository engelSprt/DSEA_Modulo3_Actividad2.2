...
int main() {
  ...
  image = fopen("imagen_entrada.bmp", "rb");      // Imagen Imagen de entrada
  outputImage = fopen("imagen_salida.bmp", "wb"); // Imagen Imagen de salida
  int desenfoque = 3; // tamaño del arreglo a promediar
  ...
  while (!feof(image)) {
    ...
    puntero[contador] = pixel;
    puntero[contador + 1] = pixel;
    puntero[contador + 2] = pixel;
    contador++;
  }
  ...
  for (int i = 0; i < alto; i++) {
    for (int j = 0; j < ancho; j++) {
      puntero[cont] = imgDesenfocada[i][j];
      puntero[cont + 1] = imgDesenfocada[i][j];
